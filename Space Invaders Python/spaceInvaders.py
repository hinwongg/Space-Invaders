...
#playerspeed = 15 #Original speed from teacher
...
#enemyspeed = 2 #Original speed from teacher
enemyspeed = 2 #Hin just for testing

special_enemyspeed = 1
...
# Hin added these "organizing" functions
def bullet_and_enemy_collision(bullet,enemy):
...
###Special Enemies Section###
##Start
def specialEnemyStamp(count): #This stamps the special enemy (bird) at the top

    special_enemy_stamp = turtle.Turtle()
    special_enemy_stamp.color("purple")
    special_enemy_stamp.shape("bird.gif")
    special_enemy_stamp.penup()
    special_enemy_stamp.speed(0)
    count = count + 20
    x = (-180) + count
    special_enemy_stamp.setposition(x, 290)
    return count
...
##Start
    for enemy in special_enemies:
        ...
        if isCollision(bullet, enemy):
            ...
            extraBullet = "yes"
            #Delete the special enemy b/c it is destroyed
            enemy.clear()
            enemy.hideturtle()
            del enemy
            #Put enemy at top of screen
            count = specialEnemyStamp(count)
        #BulletOne
        elif isCollision(bulletOne, enemy):
            os.system("afplay explosion.wav&")
            #Reset the bullet
            bulletOne.hideturtle()
            bulletstateOne = "ready"
            ...
            count = specialEnemyStamp(count)
        # BulletOne
        elif isCollision(bulletTwo, enemy):
            os.system("afplay explosion.wav&")
            # Reset the bullet
            bulletTwo.hideturtle()
            bulletstateTwo = "ready"
